...
class DICOMSequencesToMaskCustom(DICOMSequencesToMask):
    ...
    def get_coordinate_slice_from_dicom_custom(self, zip_buffer, answer=None):
        ...
        try:
            img_mesh = None
            saved_file_name, simulation_time = None, None
            front_slice, img_3d, i_slices, custom_number_slise, front_slice_ada_norm = self._search_front_slise(zip_buffer)
            ribs_detections = self._ribs_predict(front_slice)
            ...
            logger.error(f"функция get_coordinate_slice_from_dicom_custom {traceback.format_exc()}")
        return answer
